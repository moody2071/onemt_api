...
void main() {
    ...
    textCoord1.x += verticalSpeed * CC_Time.w*10.0;
    textCoord1.y += horizontalSpeed * CC_Time.w*10.0;
    vec3 color = texture2D(u_wave1, textCoord1).xyz;
	color += texture2D(u_wave2, v_texCoord).xyz;

    if(color.x > saturateValue)
        color = texture2D(u_wave1, textCoord1).xyz; //vec3(1.0);
    else
        color = texture2D(CC_Texture0, v_texCoord).xyz; 

	gl_FragColor = vec4(color, 1.0);// texture2D(CC_Texture0, v_texCoord);//.rgba; //vec4(color, 1.0);
}